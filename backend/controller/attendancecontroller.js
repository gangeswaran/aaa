...
const Teacher = require("../model/Teacher"); // Ensure you import Teacher model
const moment = require("moment"); // Optional but helps with time comparison

exports.Attendance = async (req, res) => {
  try {
    const { latitude, longitude, status, distance } = req.body;
    const { _id, dept, year, registernum } = req.student;

    // Validate if the latitude and longitude are provided
    if (!latitude || !longitude) {
      return res.status(400).json({ message: "❌ Location data is required!" });
    }
    ...
    // Fetch the teacher who has this student assigned
    const teacher = await Teacher.findOne({ assignedStudents: registernum });
    if (!teacher || teacher.allowedLocations.length === 0) {
      return res.status(403).json({ message: "❌ No location access for attendance!" });
    }
    ...
    const currentTime = moment(localNow).format("HH:mm"); // Format as HH:mm (e.g., "10:00")

    // 🕒 Check if the current time falls within the allowed time period for any of the locations
    const activeLocation = teacher.allowedLocations.find(loc => {
      return currentTime >= loc.startTime && currentTime <= loc.endTime;
    });

    if (!activeLocation) {
      return res.status(200).json({ message: "⏰ Attendance not allowed at this time!" });
    }
